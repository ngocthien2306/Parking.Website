...
namespace Modules.Pleiger.Production.Controllers
{
    ...
    public class MESProductionMagtNewController : BaseController
    {
        ...
        private const string PROJECT_STATUS_PROJECT_CLOSE = "PJST12";// Close


        public MESProductionMagtNewController(IFileService filesService, IMESSaleProjectService mesSaleProjectService, IUserService userService,
        ...
        public IActionResult ProductionPlanningDetail(string projectCode, int menuid, string vbParent)
        {
            ...
            var groupLineCount = productionService.CountGroupLine(model.ProdcnCode);
            ...
            ViewBag.GroupLineCount = groupLineCount;
            return View(model);
        }
        [HttpGet]
        public IActionResult PlanningLineGrid(string parentThread, string childThread, string projectCode, string groupLine)
        {
            ViewBag.Thread = parentThread;
            ViewBag.ChildThread = childThread;
            ProductLineGroup model = new ProductLineGroup();
            model.PlanRequestQty = 0;
            if (!String.IsNullOrEmpty(projectCode) && !String.IsNullOrEmpty(groupLine))
            {
                model = productionService.getProductionLineByGroup(projectCode, groupLine);
            };
            if (model == null)
            {
                model = new ProductLineGroup();
                model.PlanRequestQty = 0;
            }
            return PartialView("_PlanningLineGrid", model);
        }
        public IActionResult WorkDetailManagement(string projectCode, int menuid)
        {
            ...
            var lineGroups = productionService.getProductionLineGroups(projectCode);
            foreach (var group in lineGroups)
            {
                group.btnSave = data.Btn_Save;
            }
            ViewBag.LineGroups = lineGroups;
            return View(data);
        }
        [HttpGet]
        public object GetListWareHouse(DataSourceLoadOptions loadOptions)
        {
            ...
            data.Add(new MES_Warehouse { WarehouseName = "All", WarehouseCode = "All" });
        ...
        [HttpGet]
        public IActionResult GetListDataWorkPlan(DataSourceLoadOptions loadOptions, string userProjectCode, string projectName,
            string itemCode, string itemName, string customer, string SalesClassification, string projectOrderType,
            string saleOrderProjectName, string startDate, string endDate)
        {
            ...
            var data = productionService.GetListData(PROJECT_STATUS_PLAN, userProjectCode, projectName, 
            null, itemCode, itemName, customer, SalesClassification, 
            projectOrderType, saleOrderProjectName, startDate, endDate);
        ...
        [HttpPost]
        public IActionResult onSaveNew(string ProjectCode, string ProdcnCode, List<MES_ProjectProdcnLines> lstAdd,
          List<MES_ProjectProdcnLines> lstEdit, List<MES_ProjectProdcnLines> lstDelete,
          string MaterWHCode, string ProdcnMessage,
          DateTime PlanDoneDate, int[] PlanRequestQty)
        {

            //  string ProdcnCode = "ProdC1";
            var result = productionService.SaveProductPlainLinesRemake(ProjectCode, ProdcnCode, lstAdd, lstEdit, lstDelete, MaterWHCode, ProdcnMessage, PlanDoneDate, PlanRequestQty);
            return Json(new { result.Success, result.Message });
            //return Json(new { Success= true });
        }
        [HttpPost]
        public IActionResult SavePlanningFormData(string ProjectCode, string ProdcnCode, string MaterWHCode, string ProdcnMessage, DateTime PlanDoneDate)
        {
            var result = productionService.SavePlanningFormData(ProjectCode, ProdcnCode, MaterWHCode, ProdcnMessage, PlanDoneDate);
            return Json(result);
        }
        [HttpPost]
        public IActionResult SaveLineGridData(string ProjectCode, string ProdcnCode, string Data)
        {
            var result = productionService.SaveLineGridData(ProjectCode, ProdcnCode, Data);
            return Json(result);
        }
        [HttpPost]
        public IActionResult DeleteProductionLine(string ProjectCode, string ProdcnCode, string ProdcnLineCode, int GroupLine)
        {
            var result = productionService.DeleteProductionLine(ProjectCode, ProdcnCode, ProdcnLineCode, GroupLine);
            return Json(result);
        }
        [HttpPost]
        ...
        public IActionResult OnUpdateProjectReturn(string ProjectCode, string ProdcnCode, bool Flag, string ProjectStatusNow, string Planning)
        {
            ...
            if (!Flag)
            {
                remark = "Project Close(Pending Work running)";
                status = PROJECT_STATUS_PROJECT_CLOSE;//PJST12
            }
            else
            {
                remark = "Project Return(Close Work running)";
                status = PROJECT_STATUS_PROJECT_CANCEL;
            }
        ...
        [HttpPost]
        public IActionResult OnUpdateProjectReturnWorkMst(string ProjectCode, string ProdcnCode, bool Flag, string ProjectStatusNow, string Planning)
        {
            string remark = "";
            string status = "";
            //if (Planning == "Y") status = PROJECT_STATUS_PROJECT_CANCEL;
            ...
            {
                remark = "Project Return(Close Work running)";
                status = PROJECT_STATUS_PROJECT_RETURN;
            }
            var result = productionService.OnUpdateProjectReturn(ProjectCode, ProdcnCode, status, remark, ProjectStatusNow);
        ...
        {
            var data = productionService.getListProductionLine(projectCode);

            return Content(JsonConvert.SerializeObject(data), "application/json");
        }

        [HttpGet]
        public IActionResult GetListProductionLineByGroup(string projectCode, string groupLine)
        {
            var data = productionService.getProductionLineByGroup(projectCode, groupLine);

            return Content(JsonConvert.SerializeObject(data), "application/json");
        }
        [HttpGet]
        public IActionResult GetListProductionLinesDataCheckComplete(string ProjectCode, string ProdcnCode, string ProdcnLineCode, int GroupLine, int LineOrder)
        {
            var data = productionService.GetListProductionLinesDataCheckComplete(ProjectCode, ProdcnCode, ProdcnLineCode, GroupLine, LineOrder);
        ...
        [HttpPost]
        public IActionResult OnUpdateLastProdLineDoneQty(string ProjectCode, string ProdcnCode, string ProdcnLineCode, int ProductionQuantity, string ItemCode, string FinishWHCode,
            string MasterWHCode, DateTime? EndDate, DateTime? PlanStartDate, DateTime? PlanEndDate, int CumulativeProductionQuantity, string ProductionMessage, string GroupLine)
        {
            string UserID = CurrentUser.UserID;
            string UserName = CurrentUser.UserName;
            var result = productionService.OnUpdateLastProdLineDoneQty(ProjectCode, ProdcnCode, ProdcnLineCode, ProductionQuantity, UserID,
                UserName, ItemCode, FinishWHCode, MasterWHCode, EndDate, PlanStartDate, PlanEndDate, CumulativeProductionQuantity, ProductionMessage, GroupLine);
            return Json(new { result.Success, result.Message });
        }
        [HttpPost]
        public IActionResult OnUpdateLastProdLineDoneQtyAndState(string ProjectCode, string ProdcnCode, string ProdcnLineCode, int ProductionQuantity, string ItemCode, string FinishWHCode,
            string MasterWHCode, DateTime? EndDate, DateTime? PlanStartDate, DateTime? PlanEndDate, int CumulativeProductionQuantity, string ProductionMessage, string GroupLine)
        {
            string UserID = CurrentUser.UserID;
            string UserName = CurrentUser.UserName;

            var result = productionService.OnUpdateLastProdLineDoneQtyAndState(ProjectCode, ProdcnCode, ProdcnLineCode, ProductionQuantity, PRODUCTION_LINE_STATUS_DONE, UserID, UserName,
                ItemCode, FinishWHCode, MasterWHCode, EndDate, PlanStartDate, PlanEndDate, CumulativeProductionQuantity, ProductionMessage, GroupLine);
            return Json(new { result.Success, result.Message });
        }
        #endregion
