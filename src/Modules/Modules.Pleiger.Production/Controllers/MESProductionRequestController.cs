...
namespace Modules.Pleiger.Production.Controllers
{
    public class MESProductionRequestController : BaseController
    {
        ...
        private readonly IMESSaleProjectService _saleProjectService;
        public MESProductionRequestController(IHttpContextAccessor contextAccessor, IMESComCodeService mESComCodeService, IMESProductionRequestService mESProductionRequestService
            , IMESItemService mESItemService, IMESItemPartnerService mESItemPartnerService, IUserService userService, IMESSaleProjectService saleProjectService) : base(contextAccessor)
        {
            ...
            this._saleProjectService = saleProjectService;
        }
        ...
        {
            var data = mESProductionRequestService.SearchListProductionRequest(saleProject, UserCode, checkCode, StartDate, EndDate);
        ...
        // Thien add - get list file in of sale project
        [HttpGet]
        public IActionResult GetSYFileByIdProject(string projectCode)
        {
            var data = _saleProjectService.GetDataDetail(projectCode);
            var result = _saleProjectService.GetSYFileUploadByID(data.FileID);
            return Json(result);
        }
        ...
        public IActionResult UpdateDataProductionRequestChange(string projectCode, string requestCode, string requestType, string userIDRequest, string requestDate, string requestMessage , string ProductType, string ItemCode, bool InitialCode)
        {
            ...
            var result = mESProductionRequestService.UpdateDataProductionRequestChange(projectCode, RequestCode, requestType, CurrentUser.UserID, requestDate, requestMessage, ProductType, ItemCode, InitialCode);
            return Json(result);
        }
        // Quan add 2020/09/14
        ...
        {
            var result = mESProductionRequestService.RequestProduction(projectCode, CurrentUser.UserID, requestDate, requestType);
            return Json(result);
        }
        [HttpPost]
