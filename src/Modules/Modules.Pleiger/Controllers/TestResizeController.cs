...
{
    public class TestResizeController : BaseController
    {
        ...
        public TestResizeController(IHttpContextAccessor contextAccessor,
        ...
        public IActionResult Index()
        {
            ...
            if (checkUserLogin.SystemUserType.Equals("G000C005"))
            {
        ...
        {
            //check if partner và lấy list PO mà nó đã dc request
            var checkUserLogin = _userService.CheckUserType(CurrentUser.UserID);
            // Partner in groupcode SY Comcode
            if (checkUserLogin.SystemUserType.Equals("G000C005"))
            {
                var partnerCode = CurrentUser.UserCode;
        ...
        [HttpGet]
        public IActionResult ShowDetailPORequest(string projectCode, string poNumber, string partnerCode)
        {
            ViewBag.Thread = Guid.NewGuid().ToString("N");
